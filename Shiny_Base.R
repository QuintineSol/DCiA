      ...
      tabItem(tabName = "community_detection",
              fluidPage(
                ...
                div(
                  ...
                  tags$ul(
                    tags$li(strong("Fast Greedy: "), "This method quickly groups together researchers based on their collaboration patterns. Imagine you have a large group of researchers from different fields. Fast Greedy starts by considering each researcher separately and then combines them into clusters, choosing combinations that create the most cohesive groups. For example, if researchers want to foster interdisciplinary collaborations within the organization, they can use Fast Greedy to efficiently identify researchers with complementary expertise across different fields."),
                    tags$li(strong("Louvain: "), "Louvain sorts researchers into subsets where each subset contains researchers who collaborate more closely with each other than with researchers in other subsets. It starts by placing each researcher in their own subset and then merges them to maximize how well the researchers are divided. This method is great for handling large datasets and quickly finding an optimal division. For instance, if the organization wants to identify groups of researchers with similar research interests or methodologies, they can use Louvain to efficiently categorize them into clusters."),
                    tags$li(strong("Girvan-Newman: "), "This algorithm focuses on identifying critical connections between research communities within the organization. It gradually separates the network into distinct groups by removing connections that are important for maintaining the overall structure of collaboration. For example, if the organization wants to analyze collaboration networks among academic departments or research teams, Girvan-Newman can help identify key connections that bridge different research fields or groups."),
                    tags$li(strong("Walktrap :"), "nspired by the idea of random walks within a network, Walktrap discovers groups of closely connected researchers. It identifies clusters based on the strength and density of collaborations between researchers. For example, if the organization wants to explore natural groupings within their research collaboration network, Walktrap can help identify clusters of researchers with strong interconnections, facilitating the discovery of new interdisciplinary research opportunities.")
                  ),
                ),
